    ...
    riot.render = function(tagName, opts) {
        ...
        if(tag && tag[0].done) {
            return {tag: tag[0], root: root, dom: function(root) {
                console.log(sdom.serialize(root));
    ...
    _app.engine('html', function (filePath, options, cb) {
        ...
        try {
            ...
            if(view.tag && view.tag.done) {
                view.tag.on(view.tag.done, function() {
                    return cb(null, index.replace('<div id="app"></div>', '<div id="app">' + view.dom(view.root) + '</div>'));
                });
            }else {
                return cb(null, index.replace('<div id="app"></div>', '<div id="app">' + view + '</div>'));
            }
        } catch (e) {
